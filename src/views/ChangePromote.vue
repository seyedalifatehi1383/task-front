...
async function ChangeToSubAdmin(id :string) {
    const resualt = await fetch('http://localhost:3000/changeEminency/' + id,{
        method : "PATCH",
...
<template>
    <div class="backgroand">
        <div class="panelUser" v-for="user in Users" :key="user.email">
            ...
            <button @click="ChangeToSubAdmin(user.id)">change to subAdmin</button>
        </div>
    </div>
</template>
